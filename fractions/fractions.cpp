...
Fraction::Fraction(const char *str) {
  ...
  for (int i = 0; i < kBufferSize && buffer[i] != '\0'; ++i) {
    if (!std::isdigit(buffer[i]) && buffer[i] != ' ' && buffer[i] != '/' &&
        buffer[i] != '-') {
      throw std::invalid_argument("Введены недопустимые символы. Ожидаются "
                                  "цифры и символы разделителей.");
    }
  }
...
Fraction::Fraction(double value) {
  ...
  numerator =
      static_cast<int>(integerPart * scale + std::round(fractionPart * scale));
  denominator = scale;
...
Fraction Fraction::operator+(double value) { return *this + Fraction(value); }
